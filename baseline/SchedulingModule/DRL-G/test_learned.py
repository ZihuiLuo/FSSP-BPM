...
parser.add_argument('--which_benchmark', type=str, default='fssp', help='Which benchmark to test')
...
data = np.loadtxt('./BenchDataNmpy/' + benchmark + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '.txt', delimiter=' ')
#print(data.shape)
arr2d = np.array(data)
arr2d = arr2d.astype(int)
arr4d = arr2d.reshape(1, 2, N_JOBS_P, N_MACHINES_P)
#print(arr2d)
#print(arr4d.shape)
np.save('./BenchDataNmpy/' + benchmark + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '.npy', arr4d)
np.save('./DataGen/generatedData' + str(N_JOBS_P) + '_' + str(N_MACHINES_P) + '_' + 'Seed200.npy', arr4d)
input_data = np.load('./BenchDataNmpy/' + benchmark + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '.npy')
#print(input_data)
...
def test(dataset):
    ...
    t2 = time.time()
    print(t2 - t1)
    file_writing_obj = open('./' + 'Result/drltime_' + str(N_JOBS_N) + 'x' + str(N_MACHINES_N) + '_' + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '.txt', 'w')
    file_writing_obj.write(str((t2 - t1)/len(dataset)))

    # print(result)
    # print(np.array(result, dtype=np.single).mean())
    np.save('Result/drlResult_' + str(N_JOBS_N) + 'x' + str(N_MACHINES_N) + '_' + str(N_JOBS_P) + 'x' + str(N_MACHINES_P) + '_Seed' + str(SEED), np.array(result, dtype=np.single))
