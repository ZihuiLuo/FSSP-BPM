...
def load_VRP_dataset(args):
    ...
    if os.path.exists(fname):
        ...
        data = data.reshape(n_problems, n_cust, input_dim)
...
#         # data_without_norm['S3_PROCESS_TIME'] = data_without_norm[['S3_PROCESS_TIME']].apply(norm)
#         data_new = np.array(data_without_norm.values.tolist()).reshape(self.args['batch_size'], self.args['n_cust'],
#                                                               self.input_dim)
...
class Env(object):
    ...
    def each_step_reset(self, beam_width=1):# 每个训练step都要初始化一次（新的input_data）
        ...
        self.total_mask = tf.zeros([self.stage_num,self.batch_size * beam_width, self.n_nodes],
                             dtype=tf.float32)
        self.start_time = tf.zeros([self.stage_num, self.batch_size* beam_width, self.n_cust],
                                   dtype=tf.float32)
        self.end_time = tf.zeros([self.stage_num, self.batch_size* beam_width, self.n_cust],
                                 dtype=tf.float32)
    ...
    def update_start_end_time(self,idx,stage):
        ...
        def body(batch, n, end_time, start_time):
            ...
            for depot in range(self.depot_num[stage]):
                ...
                node_list = tf.map_fn(lambda node: tf.cond(tf.equal(node, self.n_cust), lambda: 0, lambda: tf.cast(node + 1, dtype=tf.int32)),
                ...
                end_time_node = tf.zeros(self.n_nodes)
    ...
    def step(self,
        ...
        # 将demand为0的顾客（即访问过的）的mask置为1，depot的mask始终为0
...
def reward_func_stage(sample_solution,stage):
    ...
    sample_solution = sample_solution[:, :, :, 2+stage] # 第0位存放demand
    # sum by node_num
...
def load_VRP_dataset(args):
    ...
    if os.path.exists(fname):
        ...
        data = pd.DataFrame(data, columns=['TOT_WGT', 'DEL_TO_HOURS', 'S1_PROCESS_TIME', 'S2_PROCESS_TIME',
                                                  'S3_PROCESS_TIME', 'S4_PROCESS_TIME'])

        data['DEL_TO_HOURS'] = data[['DEL_TO_HOURS']].apply(norm)
        data['S1_PROCESS_TIME'] = data[['S1_PROCESS_TIME']].apply(norm)
        data['S2_PROCESS_TIME'] = data[['S2_PROCESS_TIME']].apply(norm)
        data['S3_PROCESS_TIME'] = data[['S3_PROCESS_TIME']].apply(norm)
        data['S4_PROCESS_TIME'] = data[['S4_PROCESS_TIME']].apply(norm)
        data = np.array(data.values.tolist()).reshape(n_problems, n_cust, input_dim)
...
class DataGenerator(object):
    ...
    def get_train_next(self):
        ...
        data_without_norm['S2_PROCESS_TIME'] = data_without_norm[['S2_PROCESS_TIME']].apply(norm)
        data_without_norm['S3_PROCESS_TIME'] = data_without_norm[['S3_PROCESS_TIME']].apply(norm)
        data_without_norm['S4_PROCESS_TIME'] = data_without_norm[['S4_PROCESS_TIME']].apply(norm)
        data_new = np.array(data_without_norm.values.tolist()).reshape(self.args['batch_size'], self.args['n_cust'],
                                                              self.input_dim)
        depot = np.zeros((self.args['batch_size'], 1, self.input_dim))
        input_data = np.concatenate([data_new, depot], 1)
...
class Env(object):
    def __init__(self,
        ...
        self.input_pnt = self.input_data
        self.demand = self.input_data[:, :, 0]
        self.process_time = self.input_data[:,:,2:] # 各个阶段作业加工时间
        self.batch_size = tf.shape(self.input_pnt)[0]  # 批次大小
    ...
    def each_step_reset(self, beam_width=1):# 每个训练step都要初始化一次（新的input_data）
        ...
        # demand: [batch_size * beam_width, max_time]
        self.demand = tf.tile(self.demand, [self.beam_width, 1])
        self.process_time = self.input_data[:, :, 2:]
        self.ddl = self.input_data[:, :, 1][:, :self.n_cust]
        # load: [batch_size * beam_width]
        self.load = tf.ones([self.batch_beam]) * self.capacity
        # create mask,初始化全为0
