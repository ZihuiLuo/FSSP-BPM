...
class RLAgent(object):
    ...
    def build_model(self, decode_type="greedy"):
        ...
        for stage in range(args['stage_num']):
            env.each_stage_reset()
            stage_idxs = []
            stage_actions = []
            for i in range(args['n_nodes']):
                temp_idx = []
                temp_action = []
                for j in range(args['depot_num'][stage]):
                    ...
                    if decode_type == 'greedy':
                        ...
                        # batch_size_tmp = tf.shape(prob)[0]
                    #                         # node_num = tf.shape(prob)[1]
                    #                         #
                    #                         # random_select = lambda random_select: tf.random.uniform([batch_size_tmp * beam_width], 0,
                    #                         #                                                         args['n_nodes'], dtype=tf.int64)
                    #                         # init_random_select = random_select(None)
                    #                         # have_dup = lambda random_select: tf.reduce_any(
                    #                         #     tf.cast(env.mask * tf.one_hot(random_select, node_num), dtype=tf.bool))
                    #                         # idx = tf.cond(tf.random.uniform([], 0, 1) > self.epsilon,
                    #                         #               lambda: tf.argmax(prob, 1),
                    #                         #               lambda: tf.while_loop(have_dup, random_select, [init_random_select]))
            ...
            #     for k in reversed(range(len(actions_total))):
            #
            #         tmplst = [tf.gather_nd(actions_total[k],tmpind[-1])] + tmplst
            #         tmpind += [tf.gather_nd(
        ...
        R = 0.7* self.reward_func(state,env) + 0.3*variance_total * args['n_cust']
