...
```bash
python main.py --task=vrp20
```

It is possible to add other config parameters like:
```bash
python main.py --task=vrp20 --gpu=0 --n_glimpses=1 --use_tanh=False 
```
...
```bash
python main.py --task=vrp20 --is_train=False --model_dir=./path_to_your_saved_checkpoint
```
...
```bash
python main.py --task=vrp20 --is_train=False --infer_type=single --model_dir=./path_to_your_saved_checkpoint
```
### Logs
