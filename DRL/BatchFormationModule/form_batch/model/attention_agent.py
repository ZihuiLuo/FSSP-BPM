...
class RLAgent(object):
    ...
    def build_model(self, decode_type = "greedy"):
        ...
        input_pnt = env.input_pnt # input_pnt只包括坐标，没包括demand
        job_ids = env.data_id # 唯一标识一个job
        ...
        for i in range(args['decode_len']):
            ...
            identity = tf.gather_nd(tf.tile(job_ids, [beam_width, 1]), batched_idx)
            identity_tmp.append(identity)
        ...
        if decode_type=='beam_search':
            ...
            # for identity
            tmp_id_lst = []
            tmp_id = [BatchSequence]
            for k in reversed(range(len(identity_tmp))):
                tmp_id_lst = [tf.gather_nd(identity_tmp[k], tmp_id[-1])] + tmp_id_lst
                tmp_id += [tf.gather_nd(
                    (batchBeamSeq + tf.cast(batch_size, tf.int64) * beam_path[k]), tmp_id[-1])]
            identities = tmp_id_lst


        else: 
            actions = actions_tmp
            identities = identity_tmp
        ...
        #return (R, v, logprobs, actions, idxs, identities,env.input_pnt , probs)
        return (R, v, logprobs, actions, idxs, identities, job_ids, probs)
    ...
    def build_train_step(self):
        ...
        R, v, logprobs, actions, idxs , identities, batch , probs= self.train_summary
    ...
    def evaluate_single(self,eval_type='greedy'):
        ...
        for step in range(self.dataGen.n_problems):
            ...
            # R, v, logprobs, actions, idxs, identities, job_ids, probs)
            R, v, logprobs, actions,idxs, identities, job_ids, _= self.sess.run(summary,
    ...
    def evaluate_batch(self,eval_type='greedy'):
        ...
        start_time = time.time()
        R, v, logprobs, actions,idxs, identities, batch, _= self.sess.run(summary,
                                     feed_dict={self.env.input_data:data,
                                               self.decodeStep.dropout:0.0})
